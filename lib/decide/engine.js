...
const {sign}=require("../protocol/decision-token");
...
function decide({intention,modifiers=[],scope_maps=[]}){const intents=read("config/intent-registry.json").intentions,caps=read("config/capability-registry.json").capabilities;if(!intents[intention]){const request_id="decide_"+crypto.randomUUID(),now=Date.now(),decision_token=sign({request_id,intention:"UNKNOWN",capabilities:[],iat:now,exp:now+60*60*1000});return{protocol_version:"0.1",request_id,decision_token,intention:"UNKNOWN",modifiers,capabilities:[],scope_maps:{},continuations:[],limitations:[{code:"INVALID_INTENTION",message:"Unknown intention"}]};}
 ...
 let offered=Object.entries(caps).filter(([,c])=>c.intentions.includes(intention)).map(([id,c])=>({id,description:c.description,input_contract:c.input_contract,output_contract:c.output_contract}));
 ...
 const request_id="decide_"+crypto.randomUUID();const now=Date.now();const decision_token=sign({request_id,intention,capabilities:offered.map(x=>x.id),iat:now,exp:now+60*60*1000});
 return{protocol_version:"0.1",request_id,decision_token,intention,modifiers,capabilities:offered,scope_maps:returned,continuations,limitations};}
