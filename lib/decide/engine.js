...
function decide({intention,modifiers=[],scope_maps=[]}){const intents=read("config/intent-registry.json").intentions,caps=read("config/capability-registry.json").capabilities;if(!intents[intention]){const request_id="decide_"+crypto.randomUUID();return{protocol_version:"0.1",request_id,intention:"UNKNOWN",modifiers,capabilities:[],scope_maps:{},continuations:[],limitations:[{code:"INVALID_INTENTION",message:"Unknown intention"}]};}
 ...
 return{protocol_version:"0.1",request_id,intention,modifiers,capabilities:offered,scope_maps:returned,continuations,limitations};}
