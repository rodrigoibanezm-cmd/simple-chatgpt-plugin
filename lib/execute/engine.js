...
const {verify}=require("../protocol/decision-token");
...
const handlers={AGENCY_PROFILE:profile,CLIENT_RELATION:clientRelation,CLIENT_LIST:clientList,PORTFOLIO:portfolio,CREDENTIALS:credentials,CONSUMER_INSIGHTS:consumerInsights,CONTACT_DIRECTORY:contactDirectory};
function execute({request_id,decision_token,capability,scope={}}){if(!request_id||!decision_token||!capability)return fail(request_id,capability,scope,"INVALID_SCOPE","MISSING_REQUIRED","request_id, decision_token and capability are required");let decision;try{decision=verify(decision_token);}catch(e){return fail(request_id,capability,scope,"NOT_APPLICABLE","INVALID_DECISION_TOKEN",e.message);}if(decision.request_id!==request_id)return fail(request_id,capability,scope,"NOT_APPLICABLE","DECISION_REQUEST_MISMATCH","request_id does not match signed DECIDE decision");if(!Array.isArray(decision.capabilities)||!decision.capabilities.includes(capability))return fail(request_id,capability,scope,"NOT_APPLICABLE","CAPABILITY_NOT_OFFERED","Capability was not offered by DECIDE");const fn=handlers[capability];if(!fn)return fail(request_id,capability,scope,"NOT_APPLICABLE","CAPABILITY_NOT_IMPLEMENTED","Capability has no engine yet");
