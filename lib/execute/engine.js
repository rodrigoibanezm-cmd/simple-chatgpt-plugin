...
function clients(){const body=clientsText().split("Marcas y organizaciones mostradas públicamente en la página de clientes:")[1].split("## Nota")[0],assets=read("data/simple/client-assets.json").clients,workRows=works(),byClient=new Map();for(const w of workRows){if(!byClient.has(w.client_id))byClient.set(w.client_id,[]);byClient.get(w.client_id).push({work_id:w.work_id,title:w.title,youtube_url:w.youtube_url,source_url:w.source_url});}return body.split("\n").filter(x=>x.startsWith("- ")).map(x=>{const label=x.slice(2).trim(),client_id=slug(label),published_works=byClient.get(client_id)||[];return{client_id,label,logo_url:assets[client_id]?.logo_url||null,published_works};});}
