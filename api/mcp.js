...
function clientGrid(data){const rows=data.records||[];return '<div class="eyebrow">Simple / Clientes</div><div class="client-grid">'+rows.map(r=>{const works=r.published_works||[],first=works[0];return '<article class="client-card">'+(r.logo_url?'<img class="client-logo" src="'+esc(r.logo_url)+'" alt="'+esc(r.label||r.client_id)+'">':'<div class="title">'+esc(r.label||r.client_id)+'</div>')+'<div><div class="client-name">'+esc(r.label||r.client_id)+'</div>'+(first?.youtube_url?'<a class="client-work" href="'+esc(first.youtube_url)+'" target="_blank">'+(works.length>1?esc(works.length+" TRABAJOS"):"VER TRABAJO")+' →</a>':'')+'</div></article>'}).join("")+'</div>'}
